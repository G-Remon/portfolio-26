...
const ExperienceCardLeft = ({
  ...
  return (
    <div className="relative group">
      <div
        className={`absolute ${!small ? "left-1/2" : "left-0"} -translate-x-1/2 bg-gradient-to-br from-blue-500/20 to-purple-500/20 border-2 border-white/20 rounded-full p-2.5 md:p-3 z-10 backdrop-blur-sm transition-all duration-300 group-hover:scale-110 group-hover:border-white/40 group-hover:shadow-lg group-hover:shadow-purple-500/30`}
      >
      ...
      <div
        className={`relative bg-[#1f1e24] border border-white/10 hover:border-white/20 p-5 md:p-6 rounded-2xl md:rounded-3xl transition-all duration-300 hover:shadow-xl hover:shadow-purple-500/20 hover:-translate-y-1 ${
          !small ? "ml-[620px] w-[380px] md:w-[420px]" : "ml-[60px] md:ml-[70px] w-[calc(100%-60px)] md:w-[calc(100%-70px)]"
        }`}
      >
        <IoIosArrowBack
          className={`absolute ${!small ? "left-[-22px]" : "left-[-18px] md:left-[-22px]"} top-[6px] text-white/40 group-hover:text-white/60 transition-colors duration-300`}
          fontSize={!small ? 32 : 28}
        />
